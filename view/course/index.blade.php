            ...
            function () {
                ...
                table.on('tool(test)', function (obj) {
                    ...
                    switch (obj.event) {
                        ...
                        case 'show_teacher':
                            if (data.teacher) {
                                var url = '{{url('teacher/read',['id'=>'xxx'])}}'.replace('xxx', data.teacher_id);
                            ...
                            break;
                        case 'edit':
                            var url = '{{url('course/edit',['id'=>'xxx'])}}'.replace('xxx', data.id);
                            my.open('修改课程信息', url, 40, 50);
                            break;

                        case 'del':
                            layer.confirm('真的删除<span style="color:red;">' + data.name + '</span>课程么', function (index) {
                                var url = '{{url('course/delete',['id'=>'xxx'])}}'.replace('xxx', data.id);
                                $.ajax({
                                    type: 'delete',
                                    url: url,
                ...
                table.on('toolbar(test)', function (obj) {
                    ...
                    switch (obj.event) {
                        case 'add':
                            my.open('添加课程', '{{url('course/create')}}', 50);
                            break;
                    }
                    ;
                });
