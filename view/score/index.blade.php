        ...
        layui.use(['laydate', 'form', 'table'],
            function () {
                ...
                table.on('toolbar(test)', function (obj) {
                    switch (obj.event) {
                        case 'add':
                            my.open('成绩录入', '{{url('score/create')}}', 70, 90);
                            break;
                        case 'export_excel':
                            window.open('{{url('score/export')}}', '_blank');
                    }
                });
                ...
                table.on('edit(test)', function (obj) {
                    ...
                    // 在这里去后台获取课程的满分值
                    $.post("{{url('score/getfull')}}", {field: field}, function (res) {
                        var full = res.full;
