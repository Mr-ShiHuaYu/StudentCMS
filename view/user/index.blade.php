            ...
            function () {
                ...
                table.on('tool(test)', function (obj) {
                    ...
                    switch (obj.event) {
                        //监听单元格事件
                        case 'edit':
                            var url = '{{url('/user/xxx/edit')}}'.replace('xxx', data.id);
                            my.open('个人信息', url, 90);
                            break;

                        case 'del':
                            layer.confirm('真的删除<span style="color:red;">' + data.name + '</span>学生么', function (index) {
                                var url = '{{url('/user/xxx')}}'.replace('xxx', data.id);
                                $.ajax({
                                    type: 'delete',
                                    url: url,
                ...
                table.on('toolbar(test)', function (obj) {
                    switch (obj.event) {
                        case 'add':
                            my.open('添加学生', '{{url('/user/create')}}', 100, 100);
                            break;
                        case 'export_excel':
                            window.open('{{url('/user/export')}}', '_blank');
                    }
                });
